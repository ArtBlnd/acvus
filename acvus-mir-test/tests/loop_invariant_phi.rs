...
fn a_bound_and_a_factor_no_loop_assigns_are_not_block_params() {
    ...
    assert_eq!(
        ...
        "the outer header carries the accumulator, each body the counter its \
         ...
         count (RFC-0066 rule 7). Its step by `@n + 3` can overflow, so the \
         inner body keeps that step's `Check` and the loop stays \
         (RFC-0037 rule 3):\n{listing}"
    );
}
