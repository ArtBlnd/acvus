...
async fn find_resolves_on_a_string_and_on_a_regex() {
    ...
    .await;
    assert_eq!(s, "9 9", "both names answer in byte offsets");
}
...
async fn find_at_starts_where_it_is_told() {
    ...
    let s = text_of(
        ...
        r#"if let Ok(re) = regex("[0-9]+".to_string()) {
             ...
             let past = if let Some(m) = find_at(&re, t, 99u64) { m.text } else { "-".to_string() };
             first + " " + next + " " + past
           } else { "?".to_string() }"#,
    )
    .await;
    ...
    let s = text_of(
        ...
        r#"if let Ok(re) = regex("(a)|(b)".to_string()) {
             ...
             if let Some(c) = captures(&re, t) {
               ...
               let second = if let Some(m) = group(&c, 2u64) { m.text } else { "-".to_string() };
               first + " " + second
             } else { "no match".to_string() }
           } else { "?".to_string() }"#,
    )
