...
fn inline_module(
    ...
    let mut body = inline_body(&module.main, all_modules, recursive_fns);
...
fn inline_body(
    ...
    loop {
        ...
        for inst in &current.insts {
            match &inst.kind {
                InstKind::FunctionCall { dst, callee: Callee::Direct(callee_id), args }
...
fn remap_label(label: Label, offset: u32) -> Label {
    if offset == 0 { label } else { Label(label.0 + offset) }
}
...
fn remap_inst(
    ...
    match kind {
        // Constants
        InstKind::Const { dst, value } => InstKind::Const { dst: r(*dst), value: value.clone() },

        // Context
        InstKind::ContextProject { dst, id, ty } => {
            InstKind::ContextProject { dst: r(*dst), id: *id, ty: ty.clone() }
        }
        InstKind::ContextLoad { dst, src } => {
            InstKind::ContextLoad { dst: r(*dst), src: r(*src) }
        }
        InstKind::ContextStore { dst, value } => {
            InstKind::ContextStore { dst: r(*dst), value: r(*value) }
        }

        // Variables
        InstKind::VarLoad { dst, name } => InstKind::VarLoad { dst: r(*dst), name: *name },
        InstKind::VarStore { name, src } => InstKind::VarStore { name: *name, src: r(*src) },

        // Arithmetic
        InstKind::BinOp { dst, op, left, right } => {
            InstKind::BinOp { dst: r(*dst), op: *op, left: r(*left), right: r(*right) }
        }
        InstKind::UnaryOp { dst, op, operand } => {
            InstKind::UnaryOp { dst: r(*dst), op: *op, operand: r(*operand) }
        }
        InstKind::FieldGet { dst, object, field } => {
            InstKind::FieldGet { dst: r(*dst), object: r(*object), field: *field }
        }

        // Functions
        InstKind::LoadFunction { dst, id } => InstKind::LoadFunction { dst: r(*dst), id: *id },
        InstKind::FunctionCall { dst, callee, args } => {
            let callee = match callee {
                Callee::Direct(id) => Callee::Direct(*id),
                Callee::Indirect(v) => Callee::Indirect(r(*v)),
            };
            InstKind::FunctionCall { dst: r(*dst), callee, args: rv(args) }
        }
        InstKind::Spawn { dst, callee, args, context_uses } => {
            ...
            InstKind::Spawn { dst: r(*dst), callee, args: rv(args), context_uses: ctx }
        }
        InstKind::Eval { dst, src, context_defs } => {
            let ctx = context_defs.iter().map(|(id, v)| (*id, r(*v))).collect();
            InstKind::Eval { dst: r(*dst), src: r(*src), context_defs: ctx }
        }

        // Composite constructors
        InstKind::MakeDeque { dst, elements } => {
            InstKind::MakeDeque { dst: r(*dst), elements: rv(elements) }
        }
        ...
        InstKind::MakeRange { dst, start, end, kind } => {
            InstKind::MakeRange { dst: r(*dst), start: r(*start), end: r(*end), kind: *kind }
        }
        InstKind::MakeTuple { dst, elements } => {
            InstKind::MakeTuple { dst: r(*dst), elements: rv(elements) }
        }
        InstKind::TupleIndex { dst, tuple, index } => {
            InstKind::TupleIndex { dst: r(*dst), tuple: r(*tuple), index: *index }
        }

        // Pattern matching
        InstKind::TestLiteral { dst, src, value } => {
            InstKind::TestLiteral { dst: r(*dst), src: r(*src), value: value.clone() }
        }
        InstKind::TestListLen { dst, src, min_len, exact } => {
            InstKind::TestListLen { dst: r(*dst), src: r(*src), min_len: *min_len, exact: *exact }
        }
        InstKind::TestObjectKey { dst, src, key } => {
            InstKind::TestObjectKey { dst: r(*dst), src: r(*src), key: *key }
        }
        InstKind::TestRange { dst, src, start, end, kind } => {
            InstKind::TestRange { dst: r(*dst), src: r(*src), start: *start, end: *end, kind: *kind }
        }
        InstKind::ListIndex { dst, list, index } => {
            InstKind::ListIndex { dst: r(*dst), list: r(*list), index: *index }
        }
        InstKind::ListGet { dst, list, index } => {
            InstKind::ListGet { dst: r(*dst), list: r(*list), index: r(*index) }
        }
        InstKind::ListSlice { dst, list, skip_head, skip_tail } => {
            InstKind::ListSlice { dst: r(*dst), list: r(*list), skip_head: *skip_head, skip_tail: *skip_tail }
        }
        InstKind::ObjectGet { dst, object, key } => {
            InstKind::ObjectGet { dst: r(*dst), object: r(*object), key: *key }
        }

        // Closures
        InstKind::MakeClosure { dst, body, captures } => {
            InstKind::MakeClosure { dst: r(*dst), body: rl(*body), captures: rv(captures) }
        }

        // Iterator
        InstKind::IterStep { dst, iter_src, iter_dst, done, done_args } => {
            InstKind::IterStep { dst: r(*dst), iter_src: r(*iter_src), iter_dst: r(*iter_dst), done: rl(*done), done_args: done_args.iter().map(|v| r(*v)).collect() }
        }

        // Variant
        InstKind::MakeVariant { dst, tag, payload } => {
            InstKind::MakeVariant { dst: r(*dst), tag: *tag, payload: payload.map(|v| r(v)) }
        }
        InstKind::TestVariant { dst, src, tag } => {
            InstKind::TestVariant { dst: r(*dst), src: r(*src), tag: *tag }
        }
        InstKind::UnwrapVariant { dst, src } => {
            InstKind::UnwrapVariant { dst: r(*dst), src: r(*src) }
        }
        ...
        InstKind::BlockLabel { label, params, merge_of } => {
            InstKind::BlockLabel {
                label: rl(*label),
                params: rv(params),
                merge_of: merge_of.map(|l| rl(l)),
            }
        }
        InstKind::Jump { label, args } => {
            InstKind::Jump { label: rl(*label), args: rv(args) }
        }
        ...
        InstKind::Cast { dst, src, kind } => {
            InstKind::Cast { dst: r(*dst), src: r(*src), kind: *kind }
        }
...
mod tests {
    ...
    fn make_module(body: MirBody) -> MirModule {
        MirModule { main: body, closures: FxHashMap::default() }
    }

    fn v(n: usize) -> ValueId { ValueId::from_raw(n) }
    ...
    fn inline_simple_call() {
        ...
        let callee_body = make_body(vec![
            InstKind::VarLoad { dst: v(0), name: acvus_utils::Interner::new().intern("_0") },
            InstKind::BinOp { dst: v(1), op: acvus_ast::BinOp::Add, left: v(0), right: v(0) },
            InstKind::Return(v(1)),
        ], 2);

        let caller_body = make_body(vec![
            InstKind::Const { dst: v(0), value: acvus_ast::Literal::Int(1) },
            InstKind::FunctionCall { dst: v(1), callee: Callee::Direct(callee_id), args: vec![v(0)] },
            InstKind::Return(v(1)),
        ], 2);
        ...
        let has_call = inlined.main.insts.iter().any(|i| matches!(i.kind, InstKind::FunctionCall { .. }));
        assert!(!has_call, "FunctionCall should be inlined away");

        // Should have a BinOp (from callee) and a Yield.
        let has_binop = inlined.main.insts.iter().any(|i| matches!(i.kind, InstKind::BinOp { .. }));
        assert!(has_binop, "callee's BinOp should be present after inlining");

        let has_yield = inlined.main.insts.iter().any(|i| matches!(i.kind, InstKind::Return(_)));
    ...
    fn inline_preserves_extern_call() {
        ...
        let caller_body = make_body(vec![
            InstKind::Const { dst: v(0), value: acvus_ast::Literal::Int(1) },
            InstKind::FunctionCall { dst: v(1), callee: Callee::Direct(extern_id), args: vec![v(0)] },
            InstKind::Return(v(1)),
        ], 2);
    ...
    fn inline_skips_recursive() {
        ...
        let rec_body = make_body(vec![
            InstKind::Const { dst: v(0), value: acvus_ast::Literal::Int(1) },
            InstKind::Return(v(0)),
        ], 1);

        let caller_body = make_body(vec![
            InstKind::FunctionCall { dst: v(0), callee: Callee::Direct(rec_id), args: vec![] },
            InstKind::Return(v(0)),
        ], 1);
    ...
    fn inline_chain() {
        ...
        let g_body = make_body(vec![
            InstKind::Const { dst: v(0), value: acvus_ast::Literal::Int(42) },
            InstKind::Return(v(0)),
        ], 1);

        let f_body = make_body(vec![
            InstKind::FunctionCall { dst: v(0), callee: Callee::Direct(g_id), args: vec![] },
            InstKind::Return(v(0)),
        ], 1);

        let main_body = make_body(vec![
            InstKind::FunctionCall { dst: v(0), callee: Callee::Direct(f_id), args: vec![] },
            InstKind::Return(v(0)),
        ], 1);
        ...
        let has_const = inlined.main.insts.iter().any(|i| matches!(i.kind, InstKind::Const { .. }));
    ...
    fn inline_indirect_call_preserved() {
        ...
        let caller_body = make_body(vec![
            InstKind::FunctionCall { dst: v(1), callee: Callee::Indirect(v(0)), args: vec![] },
            InstKind::Return(v(1)),
        ], 2);
        ...
        let has_call = inlined.main.insts.iter().any(|i| {
            matches!(i.kind, InstKind::FunctionCall { callee: Callee::Indirect(_), .. })
        });
