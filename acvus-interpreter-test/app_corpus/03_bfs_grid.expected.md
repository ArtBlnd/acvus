...
| 49 | `for d in 0u64..4u64` (neighbours of one cell) | S1 free {`r + dr[d]`, `c + dc[d]`, bounds, cell number, `wall[next]`}; S2 Storage(`dist`) and Storage(`queue`) InOrder, no law | `dist[next]` is tested and set at a data index, and `push_back` states no law and fixes the later pop order, so the four iterations join in order |
