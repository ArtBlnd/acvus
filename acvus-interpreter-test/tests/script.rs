...
async fn combined_loop_with_conditional() {
    let i = Interner::new();
    let c = ctx(&i, &[
        ("items", Value::list(vec![Value::Int(0), Value::Int(1), Value::Int(0), Value::Int(2)])),
        ("count", Value::Int(0)),
    ]);
...
async fn combined_accumulate_product() {
    let i = Interner::new();
    let c = ctx(&i, &[
        ("items", Value::list(vec![Value::Int(2), Value::Int(3), Value::Int(4)])),
        ("sum", Value::Int(0)),
        ("product", Value::Int(1)),
    ]);
    let result = run_script(
...
async fn combined_bind_then_loop() {
    ...
    let obj = Value::object(FxHashMap::from_iter([
        (i.intern("items"), Value::list(vec![Value::Int(10), Value::Int(20)])),
    ]));
