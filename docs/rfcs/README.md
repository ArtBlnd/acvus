...
| [RFC-0050](0050-an-aggregate-is-its-components-until-it-escapes.md) | An aggregate is its components until it escapes: component registers, a wide register class (3×64 + 2×32 B) as the projection's home, the heap as the spill, multi-value return, real Rust enums at the glue | Draft |
...
| [RFC-0054](0054-the-host-declares-what-main-returns.md) | The host declares what `main` returns: the entry's declared `ret` in the graph, joined against every return by the solver's shared path, carried to `validate` as `MirModule::ret`, and no unchecked `main` | Draft |
