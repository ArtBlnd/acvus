...
| 98 | `for v in 0u64..n` (pivot: first node of `p`) | S1 free; S2 Carried(`pivot`) InOrder, law `first` with `pivot` its own guard, unset while it holds `n` (RFC-0093 rule 7); the test `part[v] == p` lies in the arm the guard decides, so it stays in the cycle's stage | `if pivot == n && part[v] == p { pivot = v }` keeps the first `v` that satisfies the test. Every write sends `pivot` a counter value, below `n` by the interval domain, so `pivot == n` holds exactly until the first hit: `pivot` is its own `\|\|` guard, and the join keeps the earlier chunk's hit, in order. The reading stops at the test: a chunk run from the unset guard computes `part[v]` where the program skips it, and the interval domain does not put `v` below `len(part)` (`part` is `filled(n, 0)`, a script closure's result), so the index can trap where the program does not. No rule discards a trap a chunk raises in work its guard decides once an earlier chunk has set the guard |
