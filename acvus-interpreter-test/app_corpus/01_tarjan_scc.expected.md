...
| 19 | `for v in 0u64..n` (csr: offsets) | S1 free {`degree[v] as u64`}; S2 Carried(`run`) scan by `+`; S3 Storage(`offs`) InOrder, law Fold(push) | `offs` receives every partial of `run`: an inclusive scan (RFC-0093 rule 8), the push joined in order by its fold law. |
...
| 102 | `for v in 0u64..n` (render) | S1 free {`comp[v].to_string()`}; S2 Storage(`out`) InOrder, law Concat | `out = out + ","` under `if v > 0` and then `out = out + …` is `out ⊕ (sep ⊕ text)`, two assignments of one storage in the iteration: the branch's other arm leaves `out` as it was, so `""` stands in (RFC-0089 rule 4), and the two combines associate into one; concatenation is not commutative, so the join is in order. The facts read `Op(Concat)` through the chain of both assignments (RFC-0089 rule 4: an assignment is read as the chain of all the iteration's assignments to the token). |
