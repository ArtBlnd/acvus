...
1. **Extern dispatch.** The interpreter reads each argument out of the register
   its operation names and hands it, by value, with `&AcvusRuntime`, to a
   handler: `SyncHandler::Arity0..Arity2` is one Rust parameter per argument,
   and `ArityN` — three or more, plus every asynchronous handler and every spawn
   — is lent a contiguous window of the caller's registers instead. The handler
   — generated by the macro — materializes each value argument, derefs each
   reference argument, and erases the return.
