...
| `Instance` | a resolved instance as one value of its own, beside the value it serves | atom — one of the runtime's values, whose word addresses an `InstanceEntry`; `I` is the requiring handler's own variable, and `call` takes the receiver at the mode `Signature::Recv` gives |
...
| `InstanceEntry` | what one instance word addresses: the glue, and the words of the instances that instance's own declaration requires | derived from `InstanceRun` — one per instance the checker settled a tree on, owned by the `Prepared` that built it, so an instance stays one value while what it requires is its own |
| `Receiver` | a `Signature::Recv` that `Instance::call` can name: `&I` or `&mut I` over a holder of the runtime's value | derived from `Signature::Recv` — the bound sits on `call` and not on the `Signature` impl, so a payload storing an `Instance` states nothing about its receiver; a by-value receiver has no impl, and the missing impl is the refusal |
...
| `InstanceEntries` | the one question `prepare` asks the registry per entry it builds: the glue of this signature's `n`-th instance | atom — the one `dyn` between `prepare` and the registry, so nothing in the crossing names a registry type |
| `InstanceTable` | the mono glue of every instance of every required signature, in the order the compiler numbers that signature's instances | derived from `InstanceAt` — the `InstanceEntries` the registry answers with; a dense row holding the glue itself and not an `Option` of one, because `Externs::combine` refuses a requirement on a signature any instance of which has none |
