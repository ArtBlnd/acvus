...
pub fn context_type(interner: &Interner) -> Ty {
    ...
    fields.insert(
        interner.intern("custom"),
        Ty::List(Box::new(custom_entry)),
    );
...
pub fn builtin_context_refs(interner: &Interner) -> FxHashSet<Astr> {
    ["turn_index", "raw", "self", "content", "context", "item", "index"]
        .iter()
...
mod tests {
    ...
    fn context_type_has_all_required_fields() {
        ...
        for name in ["system", "character", "world_info", "lorebook", "memory", "custom", "bot_name"] {
            assert!(fields.contains_key(&interner.intern(name)), "missing field: {name}");
        }
    ...
    fn context_type_custom_entry_has_type_field() {
        ...
        let Ty::List(elem) = fields.get(&interner.intern("custom")).unwrap() else { panic!() };
        let Ty::Object(custom_fields) = elem.as_ref() else { panic!() };
    ...
    fn history_entry_type_structure() {
        ...
        let Ty::Object(fields) = elem.as_ref() else { panic!() };
    ...
    fn builtin_refs_complete() {
        ...
        for name in ["turn_index", "raw", "self", "content", "context", "item", "index"] {
