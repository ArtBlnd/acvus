...
async fn run(source: &str, ret: Ty) -> i64 {
    let i = Interner::new();
    run_script(&i, source, FxHashMap::default(), ret)
        .await
        .as_int()
}
...
async fn a_reference_into_an_element_keeps_the_vector_alive() {
    let n = run(
        "let v = range(0, 10) | collect; let r = &v[3]; *r + 10",
        Ty::I64,
    )
    .await;
    assert_eq!(n, 13);
