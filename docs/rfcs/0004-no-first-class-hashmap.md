...
the expected type. Dynamically keyed data is represented as a list of key-value
pairs. An order-preserving map is that list with key uniqueness enforced at
insertion.

## Rationale

A map is an Object that has lost two distinctions: how many fields it has and
what they are named. Making that loss a first-class type invites it everywhere
and erodes the reason Object exists. A list of pairs represents dynamically
keyed data exactly, keeps insertion order, and needs no new syntax; iteration
and pattern matching already apply.
...
- Key uniqueness is a property enforced by the insertion function, not by a
  type.
...
- The exact inconvenience level of insertion, chosen so that Object remains the
  default choice.
