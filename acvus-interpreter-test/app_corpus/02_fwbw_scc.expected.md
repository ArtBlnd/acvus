...
| 98 | `for v in 0u64..n` (pivot: first node of `p`) | S1 free {`part[v] == p`}; S2 Carried(`pivot`) AnyOrder, law `min` with identity `n` | `if pivot == n && part[v] == p { pivot = v }` keeps the least `v` that satisfies the test: over increasing `v` that is `min` over the satisfying ones, identity `n`. RFC-0089 rule 4 as amended reads no such law: the branch reads the token (`pivot == n`), so it is no `last`, and `min` needs that no hit sends `n` itself, a fact about the values sent the rule does not read |
