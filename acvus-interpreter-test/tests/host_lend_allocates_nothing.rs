...
use acvus_interpreter::{
    AcvusRuntime, Host, MemoryStorage, Page, Program, Scope, SequentialExecutor, Source,
};
...
async fn page_of<'p>(scope: Scope<'p>, pairs: usize) -> Page<'p, MemoryStorage> {
    let mut page = scope.open(MemoryStorage::new()).await.expect("the init fills `@log`");
    for name in std::iter::repeat_n(["one", "two"], pairs).flatten() {
        let entry = scope.entry::<(), ()>(name).expect("the entry returns `()`");
        entry.run(&mut page, ()).await.expect("the page holds the graph's contexts");
    }
    page
}
...
fn read(page: &Page<'_, MemoryStorage>) -> Read {
...
fn a_lend_allocates_the_same_for_two_elements_and_for_sixty_four() {
    let program = program();
    block_on(program.scope(async |s| {
        let small = page_of(s, 1).await;
        let large = page_of(s, 32).await;
        read(&small);
        read(&large);

        let before = allocations();
        let small_read = read(&small);
        let small_cost = allocations() - before;
        let before = allocations();
        let large_read = read(&large);
