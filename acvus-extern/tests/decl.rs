...
use acvus_extern::{
    Arr, Eff, Effect, EffectTerm, EffectVar, ExternFn, ExternHandler, ExternItems,
    ExternRegistry, ExternType, Fn1, Interner, LenTerm, LenVar, PolyTy, Pure, RuntimeError,
    IntoValue, TyArg, TyVar, TypeRegistry, Value, extern_fn, extern_registry,
};
use std::marker::PhantomData;

#[derive(ExternType)]
#[extern_type(name = "Box")]
struct Boxed<T: TyVar, E: EffectVar>(Vec<Value>, PhantomData<(T, E)>);
...
fn identity<T: TyVar, E: EffectVar>(_: &Interner, v: T) -> T {
    v
}

#[extern_fn(effect = pure)]
fn apply<T: TyVar, U: TyVar, E: EffectVar>(_: &Interner, v: Boxed<T, E>, f: Fn1<T, U, E>) -> Boxed<U, E> {
    let _ = f;
...
#[extern_fn(effect = pure)]
#[extern_cast]
fn boxed<T: TyVar, N: LenVar>(i: &Interner, items: Arr<T, N>) -> Boxed<T, Pure> {
    Boxed(items.0.into_iter().map(|v| v.into_value(i)).collect(), PhantomData)
}
...
fn fn_ty(ty: &PolyTy) -> (Vec<PolyTy>, PolyTy, EffectTerm<acvus_extern::Poly>) {
    let PolyTy::Fn { params, ret, effect, .. } = ty else {
        panic!("not a function type: {ty:?}");
    };
    (params.iter().map(|p| p.ty.clone()).collect(), (**ret).clone(), *effect)
}

fn find<'a>(reg: &'a [acvus_mir::graph::Function], i: &Interner, name: &str) -> &'a PolyTy {
    &reg.iter().find(|f| i.resolve(f.qref.name) == name).expect(name).ty
}
...
fn generic_parameters_become_positional_variables() {
    ...
    let PolyTy::Fn { params: fparams, ret: fret, effect, .. } = &params[1] else {
    ...
    assert_eq!(params, vec![PolyTy::Array(Box::new(PolyTy::Var(0)), LenTerm::Var(0))]);
    assert_eq!(
        ret,
        PolyTy::UserDefined {
            id: acvus_extern::QualifiedRef::root(i.intern("Box")),
...
fn types_and_casts_reach_the_type_registry() {
    ...
    assert_eq!(tr.get(acvus_extern::QualifiedRef::root(i.intern("Token"))).type_params.len(), 0);

    let rules = tr.rules_to(acvus_extern::QualifiedRef::root(i.intern("Box")));
    assert_eq!(rules.len(), 1);
    assert_eq!(i.resolve(rules[0].fn_ref.name), "boxed");
    assert_eq!(rules[0].from, PolyTy::Array(Box::new(PolyTy::Var(0)), LenTerm::Var(0)));
}

fn call_sync(handler: &ExternHandler, args: Vec<Value>, i: &Interner) -> Result<Value, RuntimeError> {
    match handler {
...
async fn call_async(handler: &ExternHandler, args: Vec<Value>, i: &Interner) -> Result<Value, RuntimeError> {
    match handler {
...
async fn handlers_run_the_rust_body() {
    ...
    assert_eq!(
        call_sync(handler(&reg, &i, "add"), vec![Value::Int(40), Value::Int(2)], &i).unwrap(),
        Value::Int(42)
    );
    assert_eq!(
    ...
    let Value::Extern(o) = out else { panic!("boxed returns an extension value") };
    assert_eq!(o.type_name.name, "Box");

    let point = Value::object(
        [(i.intern("x"), Value::Int(21)), (i.intern("label"), Value::string("p"))]
            .into_iter()
            .collect(),
    );
    let out = call_async(handler(&reg, &i, "fetch"), vec![point], &i).await.unwrap();
    assert_eq!(
        Point::from_value_for_test(out, &i),
        Point { x: 42, label: "p".to_owned() }
    );
}
...
fn wrong_argument_type_is_a_runtime_error() {
    ...
    let mut tr = TypeRegistry::new();
    let reg = registry().register(&i, &mut tr);
    let err = call_sync(handler(&reg, &i, "add"), vec![Value::string("a"), Value::Int(2)], &i)
        .unwrap_err();
...
fn stand_ins_name_their_positions() {
    ...
    assert_eq!(<acvus_extern::Typeck<1> as TyArg>::poly_ty(&i, &vars), PolyTy::Var(1));
    assert_eq!(<Eff<0> as acvus_extern::EffectArg>::poly_effect(&vars), EffectTerm::Var(0));
    assert_eq!(
